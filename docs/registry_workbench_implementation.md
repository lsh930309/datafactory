...
- 분류 원천: `registry/DEEP_Agent_문서분류_레지스트리_v2.2.xlsx`
  - 문서/업무/매핑 정보는 이 워크북만 사용한다.
  - 도메인은 3번째 시트 `2.업무분류`의 `산업도메인`을 최상위 분류명으로 축약한다. 예: `금융 - 은행 (...)` → `금융`, `공공·행정 (...)` → `공공`.
  - 업무-문서 매핑이 없는 문서는 `미분류`로 표시한다.
...
- seed 발견 여부는 입고 안내에만 사용하고, 문서 진척도는 `workbench/documents/`의 manifest와 실제 산출물만으로 `미착수 / 진행중 / 완료`를 계산한다.
- 문서 현황판에는 `샘플 적재됨`, `BBox 완료`, `리뷰 완료`, `LaMa 완료`, 최종 완료 상태를 표시한다.
- 문서 ID보다 문서 제목을 먼저 표시하고, ID는 보조 badge로 노출한다.
- 레지스트리의 산업도메인/업무 연결은 도메인별 기본 목표 그룹으로 보여주되 물리 폴더는 중복하지 않는다.
