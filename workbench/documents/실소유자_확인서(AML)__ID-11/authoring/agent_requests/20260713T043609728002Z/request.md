# Agentic Authoring Request: 실소유자 확인서(AML) (ID-11)
...
## 사용자 지시
수기 문서 구형 authoring bundle을 신규 full-schema 계약으로 마이그레이션한다.
- 현재 source_review에서 status=use인 bbox와 현재 페이지 전체 이미지만 이번 마이그레이션 범위로 삼는다.
- 원본 PDF의 다른 페이지가 존재하더라도 현재 페이지에 bbox authoring이 없으면 해당 페이지 필드나 faker target을 추가하지 않는다.
- 모든 use bbox를 100% binding하고, 문서에 실제로 보이는 라벨과 구조를 기준으로 metadata 없는 계층형 semantic_schema를 만든다.
- 기존 field_id는 가능하면 유지하되 semantic_path를 문서 KIE hierarchy에 맞게 정의한다.
- 기존 bbox anchor, stylesheet, render_policy, render_mode, handwriting.qr_bbox는 적용 단계에서 보존될 자산이므로 임의로 누락하거나 대체하지 않는다.
- 기존 faker record pool에 현재 schema field 집합 밖의 p2/p3/p4 항목이 있으면 구형 잔여 데이터로 간주하고 신규 draft에 포함하지 않는다.
- faker profile은 현재 binding field_id만 포함하며 현실적 값, 체크박스 관계, 날짜/식별정보 정합성을 지원 계약 안에서 구성한다.
- 시각 정보가 최우선 source of truth이며 전체 이미지에서 정적 prefix/suffix/단위를 확인한다.
...
## 입력 파일
- sample: workbench/documents/실소유자_확인서(AML)__ID-11/samples/original/실소유자확인서.pdf
- latestReview: workbench/documents/실소유자_확인서(AML)__ID-11/review/original_실소유자확인서_page_001/review.json
- latestInpainted: workbench/documents/실소유자_확인서(AML)__ID-11/inpaint/original_실소유자확인서_page_001/lama/inpainted_lama.png
- visualEvidenceManifest: workbench/documents/실소유자_확인서(AML)__ID-11/authoring/agent_requests/20260713T043609728002Z/visual_evidence_manifest.json
