# Agentic Authoring Request: 입퇴원확인서 (MED-05)
...
## 사용자 지시
"질병분류기호" 바로 좌측에 위치한 빈 header는 무언가 적절한 header가 기입되어야 할 거 같은데, 뭐가 들어가야 적절할지 찾아봐야 할 거 같아. 또, 이 헤더 아래에 내가 임의로 3분할해둔 "병명" 테이블 같은 경우는 최대 3개의 병+질병분류기호 세트로 우선 구성하는 것으로 하자.
실질적으로 한 문서가 한 사람의 질병으로 인한 입/퇴원 상황을 보여줄 수 있게 각 key간의 연관성을 정확하게 정의해줘.
...
## 입력 파일
- sample: workbench/documents/입퇴원확인서__MED-05/samples/original/입퇴원확인서.pdf
- latestReview: workbench/documents/입퇴원확인서__MED-05/review/original_입퇴원확인서_page_001/review.json
- latestInpainted: -
- visualEvidenceManifest: workbench/documents/입퇴원확인서__MED-05/authoring/agent_requests/20260709T001741992342Z/visual_evidence_manifest.json
