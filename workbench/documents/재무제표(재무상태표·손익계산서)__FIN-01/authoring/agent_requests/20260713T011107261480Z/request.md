# Agentic Authoring Request: 재무제표(재무상태표·손익계산서) (FIN-01)
...
- 문서 ID: FIN-01
- PO 도메인: 금융, 제조, 공공, 회계
- 업무 도메인: 공공·행정 (B2G), 금융 - 은행 (여신·신용·외환), 금융 - 증권·자산운용, 제조·구매·SCM, 회계·세무 (공통)
...
- 지원 rule 문법: literal:<고정 더미 문자열>, choice:<값1>|<값2>|<값3>, pool:<data_pools 이름>, same_as:<다른 field_id>, pattern:<# 숫자, A 대문자, a 소문자, * 영문대문자/숫자 패턴>, template:<문자열과 {{지원 rule}} 조합>, person.name_ko, person.phone_kr, person.rrn, date.kr, date.year, date.month, date.day, money.krw, company.name_ko, medical.institution_ko, address.ko, free_text.short, checkbox.bool.
...
- 법령/서식상 선택지가 고정된 작은 폐쇄형 pool만 `pool_policies.<name>.closed_set:true`와 근거를 명시해 최소 크기 예외로 인정한다. 단순히 자료를 충분히 만들지 못한 pool에는 이 예외를 사용하지 않는다.
...
## 입력 파일
- sample: workbench/documents/재무제표(재무상태표·손익계산서)__FIN-01/samples/original/표준재무제표증명원＿지엘-1.pdf
- latestReview: workbench/documents/재무제표(재무상태표·손익계산서)__FIN-01/review/original_표준재무제표증명원＿지엘-1_page_001/review.json
- latestInpainted: workbench/documents/재무제표(재무상태표·손익계산서)__FIN-01/inpaint/original_표준재무제표증명원＿지엘-1_page_001/lama/inpainted_lama.png
- visualEvidenceManifest: workbench/documents/재무제표(재무상태표·손익계산서)__FIN-01/authoring/agent_requests/20260713T011107261480Z/visual_evidence_manifest.json
