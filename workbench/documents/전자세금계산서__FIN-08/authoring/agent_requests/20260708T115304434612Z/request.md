# Agentic Authoring Request: 전자세금계산서 (FIN-08)
...
## 사용자 지시
실제 값이 없는 bbox가 많기 때문에, 관련 법령이나 문서를 설명한 자료를 찾아서 정확하게 합성을 위한 데이터를 만드는 게 중요함!

## 입력 문서 컨텍스트
- 제목: 전자세금계산서
- 문서 ID: FIN-08
- PO 도메인: 공공, 무역, 회계
- 업무 도메인: 공공·행정 (B2G), 무역·물류, 회계·세무 (공통)
...
## 입력 파일
- sample: workbench/documents/전자세금계산서__FIN-08/samples/original/전자세금계산서.png
- latestReview: workbench/documents/전자세금계산서__FIN-08/review/original_전자세금계산서/review.json
- latestInpainted: workbench/documents/전자세금계산서__FIN-08/inpaint/original_전자세금계산서/lama/inpainted_lama.png
- visualEvidenceManifest: workbench/documents/전자세금계산서__FIN-08/authoring/agent_requests/20260708T115304434612Z/visual_evidence_manifest.json
